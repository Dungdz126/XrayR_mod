...
if [[ $arch == "x86_64" || $arch == "x64" || $arch == "amd64" ]]; then
    ...
    arch="64"
    echo -e "${red}Kiến Trúc Hệ Điều Hành Không Được Hỗ Trợ: ${arch}${plain}"
fi
...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        ...
        echo -e "Bản XrayR Mới Nhất：${last_version}，Bắt Đầu Cài Đặt"
    ...
    echo -e "${green}XrayR ${last_version}${plain} Quá trình cài đặt đã hoàn tất và nó đã được thiết lập để tự động khởi động khi khởi động"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "Link Dự Án Gốc：https://github.com/XrayR-project/XrayR，Cấu hình nội dung cần thiết"
    else
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR Quá trình khởi động có thể không thành công. Vui lòng sử dụng nhật ký XrayR để xem thông tin nhật ký sau. Nếu khởi động không thành công, định dạng cấu hình có thể đã bị thay đổi，请前往 wiki 查看：https://github.com/XrayR-project/XrayR/wiki${plain}"
        fi
    fi
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/Dungkobietcode/XrayR_mod/main/XrayR.sh
    ...
    rm -f install.sh
    echo -e ""
    echo "XrayR Cách sử dụng tập lệnh quản lý (tương thích với xrayr, không phân biệt chữ hoa chữ thường): "
    echo "------------------------------------------"
    echo "XrayR                    - Hiển thị menu quản trị (nhiều tính năng hơn)"
    echo "XrayR start              - Khời Động XrayR"
    echo "XrayR stop               - Tắt XrayR"
    echo "XrayR restart            - Khởi Động Lại XrayR"
    echo "XrayR status             - Hiển Thị Trạng Thái XrayR"
    echo "XrayR enable             - Bật XrayR Tự Khởi Động"
    echo "XrayR disable            - Tắt XrayR Tự Khởi Động"
    echo "XrayR log                - Hiển Thị Nhật Ký XrayR"
    echo "XrayR update             - Cập Nhật XrayR"
    echo "XrayR update x.x.x       - Cập Nhật XrayR Lên 1 Phiên Bản Cụ Thể"
    echo "XrayR config             - Chỉnh Sửa file config"
    echo "XrayR install            - Cài Đặt XrayR"
    echo "XrayR uninstall          - Gỡ XrayR"
    echo "XrayR version            - Phiên Bản XrayR Hiện Tại"
    echo "------------------------------------------"
}
